...
R (>= 4.0.0)
R Packages:rstan (for MCMC sampling)
MASS (for statistical functions)
car (for data ellipses)

Usage

Ensure your data files (X.csv, y.csv) and the Stan model file (L_cov_exp_quad_ARD5_samplesigma.stan) are in the working directory.
Configure the file paths and simulation parameters in the script:

R
X_path <- 'path/to/your/design_matrix.csv'
y_path <- 'path/to/your/response.csv'
stan_file <- 'your_model.stan'

Run the main R script. The code will:
